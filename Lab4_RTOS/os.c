...
int32_t status; 
  ...
  tcbs[7].next = &tcbs[0]; // 7 points to 0 
	...
	//specially added for the blocked variable of each thread i.e setting them all to zero means non is blocked initially  
	tcbs[0].blocked=tcbs[1].blocked=tcbs[2].blocked=tcbs[3].blocked=tcbs[4].blocked=tcbs[5].blocked = tcbs[6].blocked=tcbs[7].blocked = 0;
	//specially added for the "sleep" variable of each thread i.e setting them all to zero means non is sleeping initially  
	tcbs[0].sleep=tcbs[1].sleep=tcbs[2].sleep=tcbs[3].sleep=tcbs[4].sleep=tcbs[5].sleep = tcbs[6].sleep=tcbs[7].sleep = 0;
...
// priority on Port D edge trigger is NVIC_PRI0_R	31 – 29
 P = priority<<29; // priority shifting to get to the right position
// NVIC_PRI0_R = (NVIC_PRI0_R&0x00FFFFFF)| P;//0x60000000; //=priority 3
  NVIC_PRI0_R = (NVIC_PRI0_R&0x00FFFFFF)| 0xA0000000;//0x60000000; //=priority 3
...
// Outputs: none
 ...
 void OS_EdgeTrigger_Restart(void){
   ...
   GPIO_PORTD_IM_R |= 0x40;	
	 // clear flag6
	 GPIO_PORTD_ICR_R |=0x40; //writing 1 to the value clears it, writing a zero has no effect
}
