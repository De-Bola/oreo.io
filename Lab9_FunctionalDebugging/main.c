...
unsigned int SW1,SW2;  // input from PF4,PF0

// FUNCTION PROTOTYPES: Each subroutine defined
void DisableInterrupts(void); // Disable interrupts
void EnableInterrupts(void);  // Enable interrupts
int bothSwitchesAreNotPressed (volatile unsigned long PEData);  //Checks if SW1 is pressed
...
void PortF_Init(void){ volatile unsigned long delay;
  ...
  GPIO_PORTF_PCTL_R &= ~0x1F;   		// 4) PCTL GPIO on PF4-0
...
//Delay function that counts down to next on state
...
int main(void){  unsigned long i,last,now;//,flag;
  ...
  while(1){
    ...
    if(bothSwitchesAreNotPressed(Led)){
			Led &= ~0x02;            // switch off LED
		} else{
			//flag = 0x01; 								//initialize a manual interrupt flag
			Led ^= 0x02;             //toggle red LED			
		}
    GPIO_PORTF_DATA_R = Led;   // output 

    if ((i < 50) && ((i == 0) || (Data[i - 1] != GPIO_PORTF_DATA_R))){
			//flag &= ~flag;
      now = NVIC_ST_CURRENT_R;
			//flag = 0; //clear manual interrupt flag
			Time[i] = (last-now)&0x00FFFFFF;  // 24-bit time difference
			Data[i] = GPIO_PORTF_DATA_R&0x13; // record PF4,1,0
			last = now;
			i++;
			if(i == 50) i = 0; //to preserve continuity of array elements
    }
...
int bothSwitchesAreNotPressed (volatile unsigned long PEData){
	if((PEData & 0x11) == 0x11){ //when port data => 1 0001 and 1 0011, 
		//both switches aren't pressed due to negative logic of switches
		//but comparing both cases is redundant in this case. 
		//So, [|| PEData == 0x13] is removed from condition statement
		return 1;  //negative logic was flipped here so that line 90 would make much sense
	} else return 0;
}
// Color    LED(s) PortF
// dark     ---    0
